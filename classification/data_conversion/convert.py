...
with open('runtime.csv') as runtime_file:
    ...
    for sample in runtimes_csv:
        benchmark , loopid , trip_count , unroll_factor , time = sample
        # print benchmark , loopid , trip_count , unroll_factor , time
        time_per_trip = float(time) / float(trip_count)
        if (benchmark,loopid) in loop_to_timings:
            if unroll_factor in loop_to_timings[(benchmark,loopid)]:
                # print loop_to_timings[(benchmark,loopid)]
                (loop_to_timings[(benchmark,loopid)])[unroll_factor].append(time_per_trip)
            else:
                # print loop_to_timings[(benchmark,loopid)]
                (loop_to_timings[(benchmark,loopid)])[unroll_factor] = [time_per_trip,]
        else:
            loop_to_timings[(benchmark,loopid)] = {}
            loop_to_timings[(benchmark,loopid)][unroll_factor] = [time_per_trip,]
    

loop_to_label = {}
for benchmark , loopid in loop_to_timings:
    
    best_factor = -1
    best_time = sys.maxint
    for unroll_factor in loop_to_timings[(benchmark,loopid)]:
        curr_timing_list = loop_to_timings[(benchmark,loopid)][unroll_factor]
        curr_time = sum(curr_timing_list) / len(curr_timing_list)
        loop_to_timings[(benchmark,loopid)][unroll_factor] = curr_time
        if curr_time < best_time:
            best_time = curr_time
            best_factor = unroll_factor

    loop_to_label[(benchmark,loopid)] = best_factor
...
with open('labelled_data.csv' , 'wb') as labelled_file:
    ...
    for benchmark , loopid in loop_to_features:
        if (benchmark , loopid ) in loop_to_timings:
            ...
            X.append(features)
            label = loop_to_label[(benchmark,loopid)]
            y.append(int(label))
