...
namespace {
	...
	public:
		...
		virtual  bool runOnModule(Module &M) {
			...
			// End timer hook
			hookFunc = M.getOrInsertFunction("_Z8endTimerPcPi", \
											 Type::getVoidTy(M.getContext()),
											 PointerType::get(Type::getInt8Ty(M.getContext()), 0),
											 PointerType::get(Type::getInt32Ty(M.getContext()), 0),
											 (Type*)0);
...
namespace {
	...
	struct LoopInstrumentation : public LoopPass {
		...
		virtual  bool runOnLoop(Loop *L, LPPassManager &LPM) {
			...
			LoadInst *loopCounterExitVal = new LoadInst(loopCounterVar, loopCounterID + "_exit_val" , Exit->getTerminator());

			std::vector<Value*> args;
			args.push_back(argLoopID);
			args.push_back(dyn_cast<Value>(loopCounterExitVal));
